...
const nextNumber = appState.state.members.reduce((max, member) => {
const match = String(member.id || '').match(/TMSL-M-(\d+)/);
return match ? Math.max(max, parseInt(match[1], 10)) : max;
}, 0) + 1;

const newMember = {
...memberData,
id: `TMSL-M-${String(nextNumber).padStart(2, '0')}`,
joinedDate: new Date().toISOString().split('T')[0]
};

appState.state.members.unshift(newMember);
appState.state.activities.unshift({
id: `ACT-${Date.now()}`,
userName: appState.state.currentUser?.name || 'Admin',
action: 'Added Member',
bookTitle: newMember.name,
timestamp: 'Just now',
status: 'success'
});
appState.save();
...
<input type="text" class="form-input" id="new-member-name" placeholder="e.g. Arjun Das" required>
</div>
<div class="form-group">
<label class="form-label">Email</label>
<input type="email" class="form-input" id="new-member-email" placeholder="student@technomain.ac.in" required>
</div>
<div class="form-group">
<label class="form-label">Phone</label>
<input type="tel" class="form-input" id="new-member-phone" placeholder="+91 98765 43210" required>
</div>
<div class="form-group">
<label class="form-label">Department</label>
<select class="form-select" id="new-member-department" required>
<option value="Computer Science">Computer Science</option>
<option value="Information Technology">Information Technology</option>
...
<select class="form-select" id="new-member-role" required>
...
<select class="form-select" id="new-member-status" required>
...
const footer = `
<button class="btn btn-secondary" type="button" onclick="modal.close()">Cancel</button>
<button class="btn btn-primary" type="button" id="btn-submit-member">Add Member</button>
`;

modal.open('Add New Member', body, footer);
...
toast.show('Please fill in all member fields', 'error');
return;
}
...
try {
const member = await apiService.addMember({ name, email, phone, department, role, status });
modal.close();
toast.show(`${member.name} added successfully (${member.id})`, 'success');
renderCurrentSection();
} catch (err) {
console.error(err);
toast.show('Could not add member', 'error');
}
});
}
